...
echo "Начинаю развертывание проекта RevChecker..."
...
if [ ! -f ".env" ]; then
    ...
    echo "Создайте файл .env с API ключами для LLM моделей"
    exit 1
else
...
mkdir -p logs
mkdir -p gsheets/test_data

echo ""
echo "Развертывание завершено успешно!"
echo ""
echo "Доступные команды для запуска:"
echo "   python run_full_pipeline.py                # Основной цикл обработки"
echo "   python gsheets/fetch_reviews.py            # Получение данных из Google Sheets"
echo "   python process_reviews.py                  # Обработка отзывов через LLM"
echo "   python gsheets/update_sheets.py            # Загрузка результатов в Google Sheets"
echo ""
echo "Управление systemd сервисом:"
echo "   sudo systemctl status revchecker           # Статус сервиса"
echo "   sudo systemctl restart revchecker          # Перезапуск сервиса"
echo "   sudo systemctl stop revchecker             # Остановка сервиса"
echo "   sudo journalctl -u revchecker -f           # Логи сервиса"
echo ""
echo "Мониторинг:"
echo "   tail -f logs/*.log                         # Просмотр логов"
echo "   ps aux | grep run_full_pipeline            # Запущенный процесс"
echo ""
