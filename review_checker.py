...
async def check_review(review_text: str, gender: str = "", model: str = "grok-4-1-fast-reasoning") -> dict:
    ...
    prompt = f"""
    Ты профессиональный редактор отзывов. Задача проверить отзыв согласно входным данным и разметить орфографические ошибки.
    ВХОДНЫЕ ДАННЫЕ:
    ...
    Исправить текущий пол и окончания в тексте отзыва, если:
    ...
    Исправить грамматические ошибки, падежи, согласования, времена, стилистику, логику.
    ...
    - Знаки препинания (сохранять как есть для естественности написания отзыва человеком)
    - Буквы "е" вместо "ё" (это не ошибка)

    II. ПОМЕТКА ОРФОГРАФИЧЕСКИХ ОШИБОК:
    После проверки отзыва слова, содержащие орфографические ошибки, поместить в двойные квадратные скобки [[]].
...
if __name__ == "__main__":
    ...
    review_text = "Они нам делали оформление для юбилея ресторана. Все по самому вышему разряду подготовили. Заказывал заранее, поэтому всё успели. У гостей глаза горели, столько мне комплиментов собрали, спасибо огромное. Ну и фоточки конечно же после мероприятия остались очень красивые."
    gender = "Ж"
    model = "grok-4-0709"
